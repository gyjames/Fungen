...
def SW_match(read_info):
	#line_num, query_seq, _, query_seq_rev, _ = read_info.split()[:5]
	read_info_spt = read_info.split()
	if len(read_info_spt) < 5:#  not qualified reads
		#print('read_info', read_info, '|')
		return []
	line_num, query_seq, _, query_seq_rev, _ = read_info.split()[:5]
	line_num = int(line_num)
	query_minimizers = get_minimizers_seq(query_seq, k, w)
	query_minimizers_rev = get_minimizers_seq(query_seq_rev, k, w)
	read_counter = Counter([])
	match_num_cutoff = 1 + int(0.03 * (len(list(set(query_minimizers)))))
	related_reads = []
	for minimizer in query_minimizers:
		if minimizer in MINIMIZER_INDEX:
			read_counter.update(MINIMIZER_INDEX[minimizer])
	read_best_support = list(read_counter.most_common(1))
	query_7mer_lst = [query_seq[i : i + 7] for i in range(len(query_seq) - 7 + 1)]
	for ref_line_num, match_num in read_best_support:
		if match_num < match_num_cutoff:
			break
		if line_num == ref_line_num:
			continue
		ref_seq = MINIMIZER_DATA_CHUNK[ref_line_num - 1 - MAP_CHUNK_ORDER][1]
		match_identity, match_coverage = seq_similarity_coverage(query_seq, ref_seq)
		#print(match_identity, match_coverage, len(ref_seq), len(query_seq))
		if match_identity > 0.75 and match_coverage > 0.75:
			related_reads.append((line_num, ref_line_num, match_identity, match_coverage))
			break

	read_counter_rev = Counter([])
	match_num_cutoff_rev = 1 + int(0.03 * (len(list(set(query_minimizers_rev)))))
	for minimizer in query_minimizers_rev:
		if minimizer in MINIMIZER_INDEX:
			read_counter_rev.update(MINIMIZER_INDEX[minimizer])
	read_best_support_rev = list(read_counter_rev.most_common(1))
	query_7mer_lst_rev = [query_seq_rev[i : i + 7] for i in range(len(query_seq_rev) - 7 + 1)]
	for ref_line_num, match_num in read_best_support_rev:
		if match_num < match_num_cutoff_rev:
			break
		if line_num == ref_line_num:
			continue
		ref_seq = MINIMIZER_DATA_CHUNK[ref_line_num - 1 - MAP_CHUNK_ORDER][1]
		match_identity, match_coverage = seq_similarity_coverage(query_seq_rev, ref_seq)
		#print(match_identity, match_coverage, len(ref_seq), len(query_seq_rev))
		if match_identity > 0.75 and match_coverage > 0.75:
			related_reads.append((-line_num, ref_line_num, match_identity, match_coverage))
			break
...
def confirm_not_different_transcript(ref_seq, query_7mer_lst):
	ref_7mer_set = set([ref_seq[i : i + 7] for i in range(len(ref_seq) - 7 + 1)])
	query_7mer_in_num = 0
	for kmer in query_7mer_lst:
		if kmer in ref_7mer_set:
			query_7mer_in_num += 1
	return query_7mer_in_num / (len(query_7mer_lst) + 1)

def K_match(read_info):
	#line_num, query_seq, _, query_seq_rev, _ = read_info.split()[:5]
	read_info_spt = read_info.split()
	if len(read_info_spt) < 5:#  not qualified reads
		#print('read_info', read_info, '|')
	...
	for ref_line_num, match_num in read_best_support:
		...
		dif_transcript_ratio = confirm_not_different_transcript(ref_seq, query_7mer_lst)
		#print(match_identity, match_coverage, dif_transcript_ratio)
		if dif_transcript_ratio > 0.6:
			related_reads.append((line_num, ref_line_num))
			break
	...
	for ref_line_num, match_num in read_best_support_rev:
		...
		match_identity, match_coverage = seq_similarity_coverage(ref_seq, query_seq_rev)
		#print(match_identity, match_coverage)
		#print(cigar_seqs(ref_seq_rev, query_seq))
			...
			#print(cigar_seqs(ref_seq_rev, query_seq))
			#related_reads.append((-line_num, query_line_num))
		dif_transcript_ratio = confirm_not_different_transcript(ref_seq, query_7mer_lst_rev)
		#print(match_identity, match_coverage, dif_transcript_ratio)
		if dif_transcript_ratio > 0.6:
			related_reads.append((-line_num, ref_line_num))
			break
...
def cluster_reads_3st(args, outdir):
	start_time = time()
	start_time_t = time()
	print("\033[31m**************\033[0m", 'Recycle low quality reads', "\033[31m**************\033[0m")
	seq_record_file_lst = [int(w) for w in os.listdir('{}/seq_record'.format(outdir))]
	seq_record_file_lst.sort()

	global k, w
	k = args.k + 2
	w = args.w
	threads = args.t
	record_chunk_size = args.c
	reads_in_clusters = set([])
	ref_seq_dic = {}
	ref_minimizer_lst = []
	line_num = 0
	ref_seq_list = []
	with open ('{}/ref_seqs_2st.txt'.format(outdir)) as cluster_1st_data:
		for line in cluster_1st_data:
			line_num += 1
			line_splt = line.split()
			ref_id = int(line_splt[0])
			ref_seq = line_splt[1]
			reads_in_clusters.update([abs(int(read_name)) for read_name in line_splt[2:]])
	...
	for chunk_n in range(int((len(minimizer_data) - 1) /map_chunk_size  + 1)):
		...
		seq_file_num = 0
		low_quality_reads = []
		for seq_record_file in seq_record_file_lst:
			seq_file_num += 1
			#print(len(list(reads_in_clusters)))
			line_num = seq_record_file * record_chunk_size
			with open('{}/seq_record/{}'.format(outdir, seq_record_file)) as seq_line_data:
				for seq_line in seq_line_data:
					line_num += 1
					if line_num not in reads_in_clusters:
						low_quality_reads.append(seq_line)
			#print(seq_record_file, 'low quality read number', len(low_quality_reads))
			
			if seq_file_num % 10 == 0 or seq_file_num == len(seq_record_file_lst):
				...
				recycled_reads = p.map(SW_match, low_quality_reads)
				p.close()
				p.join()
				#print('finish')
	
				for rela_lst in recycled_reads:
					for rela in rela_lst:
						recycled_relations_uniform.append(rela)
				low_quality_reads = []
		print("\033[31m[K-mer seed map]\033[0m", 'finished {}-mer mapping in chunk {}, got {} relations, time used: {}  s'.format(k, chunk_n + 1, len(recycled_relations_uniform),  time() - start_time))
	recycled_relations_dic = {}
	for query_id, ref_id, match_iden, match_cov in recycled_relations_uniform:
		if query_id < 0:
			query_id = -query_id
			ref_id = -ref_id
		if query_id not in recycled_relations_dic:
			recycled_relations_dic[query_id] = (ref_id, match_iden, match_cov)
		else:
			if match_iden >= recycled_relations_dic[query_id][1] and match_cov > recycled_relations_dic[query_id][2]:
				recycled_relations_dic[query_id] = (ref_id, match_iden, match_cov)


	ref_cluster_dic = {}
	line_num = 0
	with open ('{}/ref_seqs_2st.txt'.format(outdir)) as cluster_2st_data:
		for line in cluster_2st_data:
			line_num += 1
			line_splt = line.split()
			#rep_id = int(line_splt[0])
			#rep_fr = int(rep_id / abs(rep_id))
			#rep_id = rep_id * rep_fr
			#ref_cluster_dic[line_num] = ([rep_fr * int(read_name) for read_name in line_splt[2:]])
			ref_cluster_dic[line_num] = [int(read_name) for read_name in line_splt[2:]]

	for i, query_id in enumerate(recycled_relations_dic):
		ref_id, match_iden, match_cov = recycled_relations_dic[query_id]
		if ref_id < 0:
			query_id = -query_id
			ref_id = -ref_id
		#if ref_id not in ref_cluster_dic:
		#	print('error')
		ref_cluster_dic[ref_id].append(query_id)
	ref_cluster_lst = [ref_cluster_dic[ref_id] for _, ref_id in enumerate(ref_cluster_dic)]

	ref_cluster_lst.sort(key = lambda x:len(x), reverse = True)
	output_file = open('{}/ref_seqs_3st.txt'.format(outdir), 'w')
	for read_lst in ref_cluster_lst:
		output_line = ''.join((''.join([str(read_id) + '\t' for read_id in read_lst]), '\n'))
	...
	print("\033[31m[Recycle reads]\033[0m", 'recycled {} low quality reads, time used: {}  s'.format(len(list(recycled_relations_dic)), time() - start_time_t))
